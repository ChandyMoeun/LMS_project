    ...
    <div class="supervisor">
      ...
      <div class="container mx-auto px-6 py-2 mt-3">
        <div class="text-right">
          <button @click="createNewPosition" class="bg-black text-white font-bold px-5 py-1 rounded focus:outline-none shadow hover:bg-yellow-400 transition-colors">
            New Position
          </button>
        </div>
            ...
            <tbody>
              <tr class="hover:bg-gray-100">
                ...
                <td class="py-4 px-6 border-b border-gray-200 text-right">
                  <button @click="editPosition(position.id)" class="font-bold py-1 px-3 rounded text-xs bg-black hover:bg-yellow-400 text-white">Edit</button>
                  <button @click="deletePosition(position.id)" class="font-bold py-1 px-3 rounded text-xs bg-red-500 hover:bg-red-400 text-white">Delete</button>
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  methods: {
    ...
    createNewPosition() {
      this.$router.push({ path: '/admin/position/create' });
    },
    editPosition(id) {
      this.$router.push({ path: `/admin/position/edit/${id}` });
    },
    deletePosition(id) {
      if (confirm('Are you sure you want to delete this position?')) {
        axios.delete(`/api/position/${id}`)
          .then(response => {
            ...
            this.getPositions(); // Refresh positions after deletion
          })
          .catch(error => {
